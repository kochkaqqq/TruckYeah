...
- `ModeratorService` — вход модератора, просмотр пользователей и объявлений, блокировка, одобрение и отклонение.
...
- Moderator: `http://localhost:5005/swagger`

Учётная запись модератора создаётся при запуске IdentityService из переменных
`MODERATOR_EMAIL`, `MODERATOR_PASSWORD`, `MODERATOR_PHONE` и `MODERATOR_POSTCODE`.
Единый API админ-панели доступен по префиксу `/api/moderator`.
